...
JSON/JSONC、HTML、CSS、JavaScript、TypeScript 和 Markdown 使用 VS Code 内建语言服务，不重复安装基础语言扩展。XML 由仍在维护的 Red Hat XML/LemMinX 负责，不采用长期未发布且依赖已废弃 `xmldom` 的 DotJoshJohnson XML Tools。拼写检查、CSS Peek 和数据库客户端不是 PHP 编码闭环的必要能力，按需单独安装；其中 Database Client 当前发行版闭源且部分功能收费，不属于本开源包。Symfony Language Tools 只补充框架感知能力；通用 Twig 解析、变量、导航和格式化仍由 TwigPlus 负责。默认关闭 `symfonyLsp.runtimeIndexing` 与 `symfonyLsp.releaseMetadata`，不执行项目内核，也不请求版本元数据。
