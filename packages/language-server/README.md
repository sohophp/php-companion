...
Composer 索引缓存使用 v45 校验封装：schema 73 的声明/签名记录、方法体实现记录、引用候选/类型依赖，以及每个 PHP 文件的 Symfony Controller/Twig 与 Doctrine 事实分别计算 SHA-256，再由整体摘要封装。所有记录核对通过后才重组语义文件；单个缓存条目损坏时只重建对应文件。这个版本建立了声明复用与方法体按需加载所需的磁盘边界，但当前查询仍完整恢复实现记录。已打开且内容不同的文档拒绝磁盘缓存，未保存内容不会写入以磁盘时间戳为键的缓存。
