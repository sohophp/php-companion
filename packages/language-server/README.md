...
Composer 索引缓存使用 v45 校验封装：schema 73 的声明/签名记录、方法体实现记录、引用候选/类型依赖，以及每个 PHP 文件的 Symfony Controller/Twig 与 Doctrine 事实分别计算 SHA-256，再由整体摘要封装。所有记录核对通过后先注册声明表，未打开文件的实现数组保持延迟；工作区类型、函数、常量和符号目录不装载正文，首次正文相关请求会原子装载目标文件的完整实现。索引日志报告剩余延迟实现数。单个缓存条目损坏时只重建对应文件；已打开且内容不同的文档拒绝磁盘缓存，未保存内容不会写入以磁盘时间戳为键的缓存。
