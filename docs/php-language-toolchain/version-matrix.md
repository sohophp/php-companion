...
| 8.4 | Property hooks、final/abstract property、非对称属性可见性、无括号 `new` 链式访问、`#[Deprecated]` | 是 | hook 局部作用域、backed/virtual、get/set 能力、setter 写入类型、非对称写可见性、接口/抽象契约、读协变/写逆变、独立 hook 继承、final 边界，以及由 `&get` 控制的数组下标修改、直接引用、唯一按引用调用和属性/对象引用遍历已进入成员语义；Deprecated 覆盖函数、方法、构造器、闭包、箭头函数、类常量、Enum case 和 hook 的已证明使用与合法目标，并统一消费 PHPDoc 标记；类、接口、Enum、普通属性、参数和匿名类目标精确拒绝，动态或歧义调用保持 unknown | 是；版本、目标、声明、继承及读写正反例具备测试 |
| 8.5 | Pipe operator、clone-with、常量表达式 callable、静态属性非对称可见性、final 提升属性、属性 `#[Override]`、`#[NoDiscard]`、`(void)` cast、Deprecated Trait/全局常量、`#[DelayedTargetValidation]` | 是；clone-with、final 提升属性与 `(void)` 使用保守兼容层 | pipe callable 链与简单 clone-with 已进入返回传播；final 提升属性进入属性事实和继承覆写检查；静态属性按读写方向应用 set 可见性；常量表达式 callable 具备 Closure 类型与非法形状诊断；属性 Override 覆盖类、接口、提升属性、匿名类及 Trait 组合，并排除私有祖先属性；NoDiscard 覆盖用户函数/方法、Trait 实际声明和已审计 DateTimeImmutable 原生方法的消费诊断，并覆盖闭包/箭头函数目标、显式返回约束与全部非法目标；Deprecated 额外覆盖 Trait use 和全局常量使用及 8.4 版本拒绝；两个原生 Attribute 均尊重同目标 DelayedTargetValidation | 是；各项具备 8.4 拒绝或无行为、8.5 接受及语义正反例 |
