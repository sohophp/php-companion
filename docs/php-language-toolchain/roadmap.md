...
- [ ] 声明与方法体分层索引，引用倒排表、派生依赖和持久缓存；更新契约已区分 declaration/implementation/none，返回具体 callable/type 身份，并按变化范围失效构造器与工厂摘要。语言服务器只在声明变化时重建 Doctrine/Symfony 声明事实。首个有界候选倒排表已随文件更新、删除和快照恢复原子替换，类型、全局函数、全局常量及静态成员 References/Rename 先缩小文件集合再执行精确语义解析；超限文档保守回退。解析后引用事实持久化、完整派生依赖图和分层磁盘格式仍待完成。
