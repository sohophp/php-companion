...
- [ ] 声明与方法体分层索引，引用倒排表、派生依赖和持久缓存；首个分层更新契约已区分 declaration/implementation/none，返回具体 callable/type 身份，并按变化范围失效构造器与工厂摘要。语言服务器只在声明变化时重建 Doctrine/Symfony 声明事实；引用倒排表、完整派生依赖图和分层持久化仍待完成。
