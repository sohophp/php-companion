...
- [ ] 声明与方法体分层索引，引用倒排表、派生依赖和持久缓存；更新契约已区分 declaration/implementation/none，返回具体 callable/type 身份，并按变化范围失效构造器与工厂摘要。语言服务器只在声明变化时重建 Doctrine/Symfony 声明事实。引用候选与类型继承依赖已作为 schema 72 的独立验证层持久化；反向依赖图随文件更新、删除和恢复原子替换，传递失效构造器摘要。每文件 Symfony Controller/Twig 上下文和 Doctrine repository/association 事实已进入 v44 校验封装，热启动不再为这些事实重跑 PHP 分析；开放文档与磁盘源不同则拒绝缓存。类型、全局函数、全局常量及静态成员 References/Rename 先缩小文件集合再执行精确语义解析；超限文档保守回退。Callable 调用依赖、Symfony YAML/编译容器等外部事实持久化及更细的声明/方法体独立磁盘记录仍待完成。
...
- [x] 移除独立推荐组合对 Intelephense 的依赖，保留明确的旧工作流兼容说明。Open Source Pack 与保留原扩展 ID 的 Recommended Pack 均安装八项开源核心工具并默认启用自研 PHP Language Server；打包校验会直接读取 VSIX manifest，拒绝 Intelephense 依赖或未启用自研核心。手动安装 Intelephense 的旧路径继续由独立兼容回归覆盖。Symfony Language Tools 0.20.1 作为可选框架增强通过组合门禁；0.20.2 的普通 PHP Rename 冲突修复前不进入无法锁定成员版本的默认 Pack。
