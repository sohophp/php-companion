...
- [ ] 声明与方法体分层索引，引用倒排表、派生依赖和持久缓存；更新契约已区分 declaration/implementation/none，返回具体 callable/type 身份，并按变化范围失效构造器与工厂摘要。语言服务器只在声明变化时重建 Doctrine/Symfony 声明事实。引用候选与类型继承依赖已作为 schema 72 的独立验证层持久化；反向依赖图随文件更新、删除和恢复原子替换，传递失效构造器摘要。每文件 Symfony Controller/Twig 上下文和 Doctrine repository/association 事实已进入 v44 校验封装，热启动不再为这些事实重跑 PHP 分析；开放文档与磁盘源不同则拒绝缓存。Symfony `services.yaml` 与新鲜 `debug-container.xml` 的未展开来源事实已进入独立 `symfony-facts-v1` 有界校验缓存，YAML resource 每次按当前 PHP 类型目录重新展开，监控变化强制绕过对应条目，单条损坏只重建该来源。类型、全局函数、全局常量及静态成员 References/Rename 先缩小文件集合再执行精确语义解析；超限文档保守回退。唯一解析、直接返回调用的工厂链已有有界反向图，被调用方变化会传递失效调用者摘要，边被移除后不再误失效；已消费的正向 Callable 构造事实进入独立 `callable-facts-v1` 缓存，按源码摘要、唯一身份和完整依赖链恢复，负向、递归、歧义及动态结论不持久化。更细的声明/方法体独立磁盘记录仍待完成。
