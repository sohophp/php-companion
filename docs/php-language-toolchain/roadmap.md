...
- [ ] 声明与方法体分层索引，引用倒排表、派生依赖和持久缓存；更新契约已区分 declaration/implementation/none，返回具体 callable/type 身份，并按变化范围失效构造器与工厂摘要。语言服务器只在声明变化时重建 Doctrine/Symfony 声明事实。schema 74 已把声明/签名、文件级实现、每个唯一函数/方法/Property Hook 的实现、引用候选与类型依赖拆为规范记录，v46 分别校验源码、声明、文件实现、每条 callable 实现、派生层和框架载荷；热缓存命中先注册声明表，类型/函数/常量/符号目录不装载正文。首次正文查询当前仍原子装载目标文件的全部实现；下一层按目标 callable 及可证明依赖装载仍待完成。反向依赖图随文件更新、删除和恢复原子替换，传递失效构造器摘要。Symfony 外部来源及已消费正向 Callable 构造事实已有独立有界缓存。类型、全局函数、全局常量及静态成员 References/Rename 先缩小文件集合再执行精确语义解析；超限文档保守回退。负向、递归、歧义及动态结论不持久化。
