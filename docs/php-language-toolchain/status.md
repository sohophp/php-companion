...
最后更新：2026-09-13。状态必须以源码和本页列出的验证命令为依据。
...
R4/F13 的 Linux x64 连续编辑、取消、重启与损坏缓存恢复证据见 [长时间编辑与恢复门禁](reports/editing-resilience-linux-x64-2026-09-13.md)。
...
2026-09-13 R4/F13 Linux x64 门禁：真实 stdio Language Server 完成 100 次预热和 1,000 次交替类型文档更新，陈旧补全为 0；更新到诊断 P95 3.38 ms，热补全 P95 1.34 ms，立即取消 1.06 ms，最终 RSS 比基线增加 3.13 MiB。实际 semantic cache 被破坏后，重启进程明确报告并重建缓存，精确补全恢复。脚本已增加 Windows/macOS RSS 采样，三系统 CI 已配置组件 tarball、500 次编辑恢复基准及真实打包 Extension Host；当前候选运行成功前不计作平台通过。WSL Remote 和多小时真实项目会话仍待系统矩阵，因此 F13 总项保持开放。
...
- [ ] P0 全部精准正反 fixtures 和真实插件组合操作核验；Open Source Profile 已在 Linux/WSL 完成隔离安装、许可证/版本、PHP/Twig/YAML/JSON formatter、EditorConfig、Xdebug 与 PHPUnit 实测，PHPUnit 快速删除日志限制已记录，Windows/macOS 和更完整 CRLF/规则组合仍待平台矩阵；标记 testkit、版本代表样例与冻结性能预算已建立，版本矩阵仍需随实现逐项关闭未支持状态。
...
当前核心编码闭环和开源组合已达到 Linux / WSL R1 首发候选；Windows/macOS 平台矩阵和 R4 最终功能仍未完成。逐项证据见 [R1 Linux / WSL 验收审计](reports/r1-acceptance-linux-wsl-2026-09-06.md)。
