...
2026-09-15 最新组合门禁：新增第三方扩展机器可读版本清单和隔离 Marketplace 安装器，CI 在 Linux、Windows 与 macOS 分别安装冻结版本并运行完整 Open Source Profile。门禁发现 Symfony Language Tools 0.20.2 会参与普通 PHP 声明 Rename 并返回 `The element can't be renamed.`；0.20.1 在相同主 VSIX、工具链及全新八扩展目录中退出码 0。由于 VS Code `extensionPack` 只能声明 ID、不能锁版本，两个默认 Pack 暂时移除 Symfony 插件，保留 0.20.1 为可选兼容组件；上游升级必须先过三平台组合门禁。Marketplace 临时 429/5xx/网络中断采用最多五次有界重试；Safe Move 协调成功前持续保留计划，并为跨平台文件系统与 LS 状态提供约 20 秒收敛窗口。Windows 客户端连接 WSL Remote 与多小时真实项目会话仍开放。证据见[Open Source Profile 版本与 Provider 组合门禁](reports/open-source-profile-version-gate-2026-09-15.md)。

2026-09-15 最新稳定性增量：Ubuntu 打包 Extension Host 两次暴露 Explorer `Service → Contact` 文件已移动但 namespace 仍旧；中间实现提交虽曾获得 15/15 CI，下一次文档提交仍复现，证明保存顺序和重试没有关闭 VS Code will-rename 附加编辑的内部失败。产品路径现让 `onWillRenameFiles` 只完成精确计划，不再向文件事务注入源文档文本编辑；`onDidRenameFiles` 与最多等待 30 秒的文件状态观察器竞争一次性领取计划，因此通知延迟或丢失也会恢复，且不会重复应用。协调仍最多重试三次，只有每个目标 namespace 已按 Composer 路径落盘且再次请求不再返回任何编辑时才成功。16 个组件 624 项与根扩展 35 项测试通过；同一主 VSIX 连续两个全新隔离 Profile 均通过完整宿主回归。提交 `35773d5` 的 [CI 34873469728](https://github.com/sohophp/php-companion/actions/runs/34873469728) 15/15 成功，覆盖三平台打包宿主、质量/产物门禁及 PHP 7.2–8.5 运行时集成。证据见[Safe Move 移动后收敛验收](reports/safe-move-reconciliation-retry-2026-09-15.md)。
