...
2026-09-15 最新稳定性增量：Ubuntu 打包 Extension Host 两次暴露 Explorer `Service → Contact` 文件已移动但 namespace 仍旧；中间实现提交虽曾获得 15/15 CI，下一次文档提交仍复现，证明保存顺序和重试没有关闭 VS Code will-rename 附加编辑的内部失败。产品路径现让 `onWillRenameFiles` 只完成精确计划，不再向文件事务注入源文档文本编辑；`onDidRenameFiles` 与最多等待 30 秒的文件状态观察器竞争一次性领取计划，因此通知延迟或丢失也会恢复，且不会重复应用。协调仍最多重试三次，只有每个目标 namespace 已按 Composer 路径落盘且再次请求不再返回任何编辑时才成功。16 个组件 624 项与根扩展 35 项测试通过；同一主 VSIX 连续两个全新隔离 Profile 均通过完整宿主回归。新跨平台 CI 待候选提交补齐。证据见[Safe Move 移动后收敛验收](reports/safe-move-reconciliation-retry-2026-09-15.md)。
